...
Sigfunc * Signal( int signo, Sigfunc *func) {
  ...
  act.sa_handler = func;
  sigemptyset (&act.sa_mask); // outros sinais nao sao bloqueados
  act.sa_flags = 0;
...
int main (int argc, char **argv) {
   ...
   servaddr.sin_port        = htons(port);   // Porta como argumento
